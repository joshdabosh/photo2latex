...
async function loadModel(id){
	if (tf.ready()){
		...
		net = await tf.loadLayersModel('https://raw.githubusercontent.com/Qwerty71/codein/Qwerty71-patch-1/model.json');
		console.log("model loaded...");

		let offset = tf.scalar(255);
		document.getElementById('status').innerHTML = "Status: Predicting..."
		tensor = tf.browser.fromPixels(img).resizeNearestNeighbor([150, 150]).toFloat().div(offset);
		...
		<div>
					...
					{cards.map((card, idx) => (
						<Card key={idx}>
							<Card.Body>
								...
								<Card.Img id={card} variant="top" src={Holder} />
			...
			<Modal show={show} onHide={handleClose}>
				...
				<Modal.Body>
					...
					<br></br>
					<p id='status' style={{fontSize: "50%"}}> Status: Waiting for upload...</p>
					<p id='test'>Symbol:  </p>
				</Modal.Body>
				<Modal.Footer>
			...
			<img src={Testing_Image} id="a_137.jpg" style={{display: "none"}}/>
			<img src={Testing_Image2} id="c_14147.jpg" style={{display: "none"}}/>
		</div>
	)
}
